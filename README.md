...
- [User Stories、Acceptance Criteria 與 Expect 對應](#user-storiesacceptance-criteria-與-expect-對應)
  ...
  - [User Story 2：依時間區間查看熱門文章排行](#user-story-2依時間區間查看熱門文章排行)
    - [AC 2.1：切換時間區間後，畫面顯示目前選取區間](#ac-21切換時間區間後畫面顯示目前選取區間)
    - [AC 2.2：排行列表與 API 回傳資料一致](#ac-22排行列表與-api-回傳資料一致)
    - [AC 2.3：所有支援的時間區間皆可正確切換](#ac-23所有支援的時間區間皆可正確切換)
- [CI](#ci)
- [注意事項](#注意事項)
...
| Acceptance Criteria | Playwright Expect |
| --- | --- |
| 每個時間區間皆能正確切換 | `expectTimeRangeButtonLabel(label)` 與 `expectTimeRangeLabel(label)` |
| 每個時間區間的排行資料皆與 API 一致 | `expectItemsToMatch(rankings)` |
